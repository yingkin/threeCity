...
export function addLightLine(shader) {
  ...
  //设置扩散条带的宽度
  shader.uniforms.uLightLineWidth = { value: 60 };
  ...
  shader.fragmentShader = shader.fragmentShader.replace(
    ...
    `
      ...
      if(LightLineMix>0.0){
        gl_FragColor = mix(gl_FragColor,vec4(1,0.8,0.9,1),LightLineMix/uLightLineWidth);
      }
    //#end#
    `
  );
...
export function addToTopLine(shader) {
  ...
  //设置扩散条带的宽度
  shader.uniforms.uToTopWidth = { value: 60 };
